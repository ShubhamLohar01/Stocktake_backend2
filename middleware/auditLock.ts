 ...
 */
export async function blockFloorManagerWritesDuringAudit(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (user.role !== "FLOOR_MANAGER") return next();
  ...
  try {
    ...
    if (req.body?.warehouse) warehouseName = req.body.warehouse;

    // Try to get from params (e.g., /api/warehouses/:warehouseId/...)
    if (!warehouseId && req.params?.warehouseId) warehouseId = req.params.warehouseId;
    ...
    return res.status(423).json({
      ...
      message:
        "An audit session is in progress for this warehouse. Floor managers cannot edit, insert, or update stock entries until the audit is completed.",
