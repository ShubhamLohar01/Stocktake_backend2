...
export const getWarehouseAudits = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
...
export const getOrCreateFloorSession = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
...
// Get floor session
export const getFloorSession = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
...
export const submitFloorSession = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        const { sessionId } = req.params;
...
export const approveFloorSession = async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: "Unauthorized" });
        if (req.user.role !== "INVENTORY_MANAGER" && req.user.role !== "ADMIN") {
